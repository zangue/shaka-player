...
shaka.routing.Walker = class {
  ...
  constructor(startingAt, startingWith, implementation) {
    console.log(`[lgt] Init walker at ${startingAt.name}`, startingWith);
  ...
  doOneThing_() {
    ...
    console.log(`[lgt] idle, waiting for new route request...`);
  ...
  tryNewRoute_() {
    ...
    if (this.currentRoute_) {
      console.log(`[lgt] --- cancel current route: ${this.currentStart_.name} -> ${this.currentRoute_.node.name}`);
      this.currentRoute_.listeners.onCancel();
      this.currentRoute_ = null;
      this.currentStart_ = null;
    }
    ...
    if (newRoute) {
      console.log(`[lgt] +++ starting new route: from ${this.currentlyAt_.name} -> ${newRoute.node.name}`);
      ...
      console.log(`[lgt] skip routing attempt.`);
  ...
  async takeNextStep_() {
    ...
    console.log(`[lgt] step: <${this.currentlyAt_.name}> (active route: ${this.currentStart_.name} -> ${this.currentRoute_.node.name})`);
    ...
    try {
      ...
      if (this.currentlyAt_ == this.currentRoute_.node) {
        console.log(`[lgt] Completed route: ${this.currentStart_.name} -> ${this.currentRoute_.node.name}`);
      ...
      if (error.code == shaka.util.Error.Code.OPERATION_ABORTED) {
        ...
        console.log(`[lgt] Aborted route: ${this.currentStart_.name} -> ${this.currentRoute_.node.name}`);
        this.currentRoute_.listeners.onCancel();
      } else {
        ...
        console.log(`[lgt] Routing error at ${this.currentlyAt_.name}: ${this.currentStart_.name} -> ${this.currentRoute_.node.name}`);
      ...
      this.currentlyAt_ = await this.implementation_.handleError(
          this.currentlyWith_,
          error);
      console.log(`[lgt] Reset load graph after error. Move to <${this.currentlyAt_.name}>`)
    }
  }
