...
/**
 * @event shaka.Player.BufferedToEndEvent
 * @description Fired when the player has buffered enough data to play the media
 *   up to its end without interruption.
 * @property {string} type
 *   'bufferedtoend'
 * @exportDoc
 */
...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  pollBufferState_() {
    ...
    if (bufferedToEnd) {
      const eventName = shaka.util.FakeEvent.EventName.BufferedToEnd;
  ...
  isBufferedToEndMS_() {
    ...
    const videoEnd = this.mediaSourceEngine_.bufferEnd('video');
    if (videoEnd >= this.video_.duration) {
      console.log('MS.ended', this.mediaSourceEngine_.ended());
      this.video_.pause();
      debugger;
    }
