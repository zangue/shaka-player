...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  constructor(mediaElement, dependencyInjector) {
    ...
    const walkerImplementation = {
      ...
      handleError: async (has, error) => {
        ...
        // TODO
        return has.mediaElement || !has.useSrcEquals ? this.mediaSourceNode_ : this.detachNode_;
      },
      onIdle: (node) => {
        this.dispatchEvent(this.makeEvent_(
  ...
  attach(mediaElement, initializeMediaSource = true) {
    ...
    if (this.walker_.getCurrentNode() == this.detachNode_) {
      ...
      //   // TODO
      //   // Double check this.
      //   // Test case: attaching ignores init media source flag
    ...
    if (destination) {
      const events = this.walker_.startNewRoute((currentPayload) => {
        console.log('<<<<<<<< atach start route ', currentPayload, this.currentLoadPayload_);
        return {
          node: destination,
  ...
  async load(assetUri, startTime, mimeType) {
    ...
    const events = this.walker_.startNewRoute((currentPayload) => {
      ...
      if (currentPayload.mediaElement == null) {
        console.log('>>>>>>>>>>>>>> No media element on load');
  ...
  getNextStep_(currentlyAt, currentlyWith, wantsToBeAt, wantsToHave) {
    ...
    if (currentlyAt == this.unloadNode_) {
      // if (wantsToBeAt == this.detachNode_) {
      //   next = this.detachNode_;
      // } else if (currentlyWith.mediaElement) {
      //   next = this.shouldUseSrcEquals_(currentlyWith) ? this.attachNode_ : this.mediaSourceNode_;
      // } else {
      //   next = this.detachNode_;
      // }
